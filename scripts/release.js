...
/**
 * Automatically bumps the package version according to the specified version
 * parameter.
 *
 * The given version parameter can have the following values: "recommended",
 * "first", major", "minor", "patch", or a valid semantic version string.
 *
 ...
 *
 * If the version is "first", the package version is bumped to the current
 * version specified in package.json.
 *
 * If the version is "major", "minor" or "patch", the package version is
 * incremented to the next major, minor, or patch version, respectively.
 *
 * If the version is a valid semantic version string, the package version is
 ...
 *
 * Returns a promise that resolves to the bumped version, or a promise that is
 * rejected in case an error occured.
 */
    ...
    return new Promise((resolve, reject) => {
        ...
        if (version === "recommended") {
            conventionalRecommendedBump({
                ...
                if (error) {
                    reject(new Error(`conventional recommended bump failed: ${error}`));
                    return;
                }
                bumpVersion(recommendation.releaseType, true, true);
            });
            return;
        }
...
/**
 * Updates the CHANGELOG.md on the current branch with release information from
 * the conventional commits, commits all pending changes and creates an
 * annotated git tag for the release.
 *
 * If supplied with releaseNote, this text is prepended to the release
 * information generated by conventional commits.
 */
...
/**
 * Push to current branch with the release tag to the remote git repo.
 *
...
/**
 * Publishes all the distribution packages of the new release on an npm
 * registry. Authentication is required to publish packages. The registry
 * location is determined by the `publishConfig` section in the package.json, if
 * present. If not present, the public npm registry is used.
 *
 * Registers the published packages with the given tag, such that npm install
 * <name>@<tag> will install this version. By default, `npm install` installs
 * the `latest` tag.
 */
function publishRelease(npmDistTag) {
    ...
    return Promise.resolve()
        ...
        .then(() => asyncInSeries(distFolders, folder => runCommandWithRedirectedIo(`npm publish "${folder}" --access public --tag ${tag}`)))
...
/**
 * *For testing purposes only*: Updates the CHANGELOG for the given package
 * version with the given release information (optional) from the conventional
 * commits.
 *
 * If supplied with releaseNote, this text is prepended to the release
 * information generated by conventional commits.

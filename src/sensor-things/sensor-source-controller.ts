...
export class SensorSourceController extends Controller {
    ...
    registerSensor(
        ...
        if (!this.options["skipSensorAdvertise"]) {
            this.communicationManager
                .publishAdvertise(AdvertiseEvent.withObject(sensor));
        }
    ...
    unregisterSensor(sensorId: Uuid) {
        ...
        if (!this.options["skipSensorDeadvertise"]) {
            this.communicationManager
                .publishDeadvertise(DeadvertiseEvent.withObjectIds(sensorId));
        }
     ...
     * @param featureOfInterestId UUID of associated feature of interest.
     * (optional)
     */
        ...
        this._querySubscription = this.communicationManager
            ...
            .subscribe(event => {
                ...
                if (retrieved.length > 0) {
                    event.retrieve(RetrieveEvent.withObjects(retrieved));
                }
            });
    }
        ...
        container.io.read(value => {
            ...
            if (channeled) {
                this.communicationManager.publishChannel(
                    ChannelEvent.withObject(this.getChannelId(container), observation));
            } else {
                this.communicationManager.publishAdvertise(
                    AdvertiseEvent.withObject(observation));
            }
